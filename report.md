...
- Complexity: If a file exists in the current branch but not the target, it must be deleted. If it exists in both but differs, it must be overwritten. This requires comparing two full trees and applying the diff to the working directory
...
5. This uses only the index and object store - no diff needed
...
- Recovery: Run git log before switching to note the hash, then create a branch: git branch recovery-branch <hash>
...
- Data structure: A hash set for O(1) lookup
- Estimate: 100,000 commits x 3 objects each = 300,000 objects to visit across 50 branches
...
4. GC deletes the blob because it was not reachable at scan time
